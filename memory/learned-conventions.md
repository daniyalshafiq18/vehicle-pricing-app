...
- **DriveArabia per-year pages mix multi-trim prices with one selected-trim spec block** (2026-08-13) — visible copy mentions several engines/fuels and can produce a plausible but wrong spec (a selected petrol Camry was read as Hybrid). Prefer the schema.org `Product`/`Vehicle` block and require its `vehicleConfiguration` + year to exactly match the MVR before writing specs. Other trims on the same page receive prices only.
