...
During Phase 4 live testing, PAD processing is temporarily record-scoped in the MVR modal. **Process PAD Capture** invokes the existing inbox processor with only the open request; the global page action is hidden. This improves testing clarity but does not add random-access queue selection: the relay still returns the oldest Pending capture, which remains Pending when it belongs to another record.
