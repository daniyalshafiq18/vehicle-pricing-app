...
During live testing, PAD processing is temporarily record-scoped in the MVR modal. **Process PAD Capture** requires the 12-character Inbox ID returned by PAD and invokes the existing inbox processor with that exact ID plus only the open request; the global page action remains hidden. This prevents an unrelated older Pending capture from blocking or being compared with the selected record.

The per-request admin **Scrape** action now opens a source-selection dialog with YallaMotor and DriveArabia enabled by default. `executeMultiSourceScrape()` prepares the shared evidence contract first, dispatches YallaMotor into its prepared target, and returns the correlated DriveArabia PAD URL to the dialog. Immediate-source errors are returned per source rather than rejecting away the pending attended work. The existing bulk action remains the compatibility YallaMotor-only path and is labeled accordingly.
...
Phase 7A invokes attended DriveArabia PAD only through a Power Pages-associated cloud flow at `/_api/cloudflow/v1.0/trigger/<guid>`. Production resolves that GUID at page-render time from the `VPI/DriveArabiaCloudFlowId` site setting exposed as `window.vpiRuntimeConfig` by both code-site entry documents (`Home.webpage.copy.html` and `SPA-Shell`); `VITE_DRIVEARABIA_CLOUD_FLOW_ID` remains only a local-development/build fallback. Replacing a flow registration therefore requires a site-setting update and cache/sync refresh, not another SPA build. `driveArabiaPadAutomation.ts` sends the fixed outer `eventData` API envelope as a CSRF-authenticated, URL-encoded form POST through `shell.ajaxSafePost` when available or an equivalent `shell.getTokenDeferred()` fallback. Current Blank Template runtimes can resolve that token API to a hidden input element, so the adapter extracts its `value` instead of serializing the element into the request header. The envelope contains four inner properties: correlated `driveArabiaUrl`, `missingVehicleRequestId`, `runCorrelationId`, and `attemptNumber`. The cloud-flow endpoint must not reuse the Dataverse `webapi.safeAjax` JSON transport. Those names and types must exist directly as Power Pages trigger inputs; `eventData` itself is not a trigger input, and the flow does not Parse JSON. The flow must be solution-aware, assigned only to the administrator web role for production, and return PAD's `InboxId` plus `StatusCode=202`. The app then processes that exact inbox row; it never drains an unrelated oldest item during the automated handoff. Missing configuration or any automation failure preserves the attended correlated URL as rollback. A 2026-08-25 development-only exception temporarily assigns the flow to Authenticated Users; Anonymous access remains forbidden and the exception must be removed before production.
